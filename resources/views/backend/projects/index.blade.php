...
@section('title')
{!! trans('system.action.list') !!} {!! trans('projects.label') !!}
@stop
...
<section class="content overlay">
    ...
    <div class="box box-default">
        ...
        <div class="box-body">
            {!! Form::open(array('url' =>route('admin.projects.index') , 'role'=>'search', 'method' => 'GET')) !!}
            <div class="row">
                <div class="col-md-3">
                    <div class="form-group">
                        {!! Form::label('title', trans('projects.title')) !!}
                        {!! Form::text('title', Request::input('title'), ['class' => 'form-control']) !!}
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        {!! Form::label('category_id', trans('projects.category')) !!}
                ...
                <div class="col-md-2">
                    <div class="form-group">
                        {!! Form::label('featured', trans('projects.featured')) !!}
    ...
    <div class="row">
        <div class="col-md-2" style='float: left;'>
            <a href="{!! route('admin.projects.create') !!}" class='btn btn-primary btn-flat'>
    ...
    @if (count($projects) > 0)
        ...
        <div class="box">
            <?php $i = (($projects->currentPage() - 1) * $projects->perPage()) + 1; ?>
                ...
                <table class="table table-striped table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align: center; vertical-align: middle;">#</th>
                            <th style="text-align: center; vertical-align: middle;"> {!! trans('projects.title') !!} </th>
                            <th style="text-align: center; vertical-align: middle;"> {!! trans('projects.featured') !!} </th>
                            <th style="text-align: center; vertical-align: middle;"> {!! trans('projects.image') !!} </th>
                            <th style="text-align: center; vertical-align: middle;"> {!! trans('projects.category') !!} </th>
                            ...
                            <th style="text-align: center; vertical-align: middle;"> {!! trans('projects.created_by') !!} </th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td style="text-align: center; width: 3%; vertical-align: middle;">
                                {!! Form::checkbox('projectsId', $item->id, null, array('class' => 'projectsId')) !!}
                            </td>
                            <td style="text-align: center; vertical-align: middle;">{!! $i++ !!}</td>
                            <td style="text-align: justify; vertical-align: middle;">
                                {!! HTML::link( route('admin.projects.edit', $item->id), \App\Helper\HString::modSubstr($item->title, 50), array('class' => '', 'title' => $item->title)) !!}
                            </td>
                            ...
                            <td style="text-align: center; vertical-align: middle;">
                                @if($item->image)
                                <img src="{!! asset('assets/media/images/projects/' . $item->image) !!}" height="50px" style="max-width: 80px;">
                                @endif
                            </td>
                            <td style="text-align: center; vertical-align: middle;">
                                <span class="label label-{!! $labels[ $item->category_id % 5 ] !!}">{!! \App\Models\ProjectCategory::find($item->category_id)->name !!}</span>
                            </td>
                            <td style="text-align: center; vertical-align: middle;">
                                @if($item->status == 0)
                                <span class="label label-danger"><span class='glyphicon glyphicon-remove'></span></span>
                            ...
                            <td style="text-align: center; vertical-align: middle;">
                                <a href="{!! route('admin.projects.edit', $item->id) !!}" class="text-warning"><i class="glyphicon glyphicon-edit"></i> Sửa </a>
                                &nbsp;
                                <a href="{!! route('admin.projects.delete', $item->id) !!}" class="btn-confirm text-danger"><i class="glyphicon glyphicon-remove"></i> Xóa </a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
...
<script>
    !function ($) {
        $(function(){
            ...
            var countChecked = function() {
                $( "#counterSelected" ).text( $( "input[name='projectsId']:checked" ).length );
            };

            countChecked();

            $( "input[type=checkbox][name='projectsId']" ).on( "click", countChecked );
    ...
    function save() {
        if($( "input[name='projectsId']:checked" ).length == 0) {
        ...
        $.each($("input[name='projectsId']:checked"),
            function () {
                values.push($(this).val());
            });

        $.ajax({
            url: "{!! URL::route('admin.projects.updateBulk') !!}",
